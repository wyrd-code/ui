...
import type { Timeout } from '@/domain'
...
export default defineComponent({
  ...
  setup(props, { emit }) {
    ...
    function calculateNewValue(
      ...
      const newValue =
        stepType === 'plus' ? currentValue + step : currentValue - step
      if (newValue > props.max) {
        return props.max
      }
      return newValue < props.min ? props.min : newValue
    }
    ...
    function calculateStep(stepType: 'plus' | 'minus') {
      ...
      const step = Number(props.step)
      const newValue = calculateNewValue(stepType, currentValue, step)
      emit('update:model-value', Number(newValue.toFixed(10)))
    }
