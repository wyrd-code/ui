  ...
  >
    ...
    <div class="flex example-btn">
      <FileUpload
        ref="upload"
        v-model="files"
        class="border rounded-full font-medium bg-gray-500 border-gray-500 shadow-sm mr-2 text-white text-sm py-2 px-4 inline-flex justify-center hover:bg-gray-600 focus:border-yellow-300"
...
export default defineComponent({
  ...
  setup(_, { emit }) {
    ...
    const isSystemFile = (filename: string) =>
      /(\/|^)(Thumbs\.db|desktop\.ini|\..+)$/.test(filename)
    const isCodeFile = (filename: string) =>
      /\.(js?|ts?|html?|jsx?)$/i.test(filename)
    ...
    function inputFilter(
      ...
      if (oldFile || !newFile?.name) {
        return
      }

      if (isCodeFile(newFile?.name) || isSystemFile(newFile?.name)) {
        return prevent()
      }
    }
